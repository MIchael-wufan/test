 ...
 * 乘法竖式：被乘数 × 乘数，逐位相乘后求和
 */
function generateMulLatex(multiplicand: number, multiplier: number): string {
 ...
 * 格式：商在上，被除数在中，逐步相减展示
 */
function generateDivLatex(dividend: number, divisor: number): string {
  ...
  // 逐步长除法步骤
  const steps: Array<{ sub: string; rem: string }> = [];
  let current = 0;
  let qDigits = "";
      ...
      res.on("end", () => {
        try {
          ...
          if (json.url || json.upload_url) {
            ...
            else reject(new Error(`Cannot parse URL from: ${json.markdown}`));
          } else {
